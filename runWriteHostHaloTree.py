	...
	def __init__(self, configfile):
		...
		with open(configfile,"r") as f:
			...
			for line in f:
				...
				if line[0] in ['Orbweaver_path', 'Velcopy_path', 'Output_path', 'Output_name']:
...
for i in ht.halotree.keys():
	...
	if (ht.halotree[i].hp['hostHaloIndex'] is None) or (len(ht.halotree[i].hp['hostHaloIndex']) != len(ht.halotree[i].hp['hostHaloID'])):
...
for i in range(len(mass_bins) - 1):
	...
	oi = ow.OrbitInfo(owd, hosts=hosts_temp, physical=False, max_times_r200=4, 
		npart_list=ht.halotree[ht.snapend].hp['npart']*ht.halotree[ht.snapend].hp['Efrac'], skeleton_only=True)
	...
	writeDataToHDF5(outpath, outname+'%i.hdf5'%i, ht, hosts, mainhaloes, mh_tree, oi.output_notmerged, oi.output_merged, nbtree, nbtree_merger)