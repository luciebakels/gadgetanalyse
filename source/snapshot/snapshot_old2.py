...
class Snapshot:	
	def __init__(self, path, nsnap, partType=1, d_partType = None, useIDs=True, conversions=[1, 1, 1, 1], nfiles=1, nfilestart=0, debug=False,
		softeningLength = 0.002, bigFile=False, read_only_coords=False, read_only_header=False, readIDs=True,
		snapshottype='GADGET'):
		...
		for pT in self.readParticles:
			...
			for n in self.dataopen.keys():
				...
				coordinates = self.dataopen[n]['PartType{}/Coordinates'.format(int(pT))][:,:]*self.lengte_to_Mpc
				endnumber = startnumber + len(coordinates)
				
				if not read_only_coords:
					if self.snapshottype == 'gadget':
						velocities = self.dataopen[n]['PartType{}/Velocities'.format(int(pT))][:,:]*self.snelheid_to_kms*np.sqrt(1./(1.+self.redshift))
					else:
						velocities = self.dataopen[n]['PartType{}/Velocities'.format(int(pT))][:,:]*self.snelheid_to_kms
					...
					if self.mass[int(pT)] != 0:
						...
						masses = self.dataopen[n]['PartType{}/Masses'.format(int(pT))][:]*self.massa_to_10msun
					if self.namePrefix[i] == 'H':
						internalEnergy = self.dataopen[n]['PartType{}/InternalEnergy'.format(int(pT))][:]*self.massa_to_10msun*self.snelheid_to_kms**2
						density = self.dataopen[n]['PartType{}/Density'.format(int(pT))][:]*self.dichtheid_to_Mpc3
		...
		if self.bigFile:
			...
			for pT in self.readParticles:
				if self.mass[int(pT)] == 0:
					...
					masses = self.dataopen['PartType{}/Masses'.format(int(pT))][:]*self.massa_to_10msun
	...
	def get_temphalo(self, coord, radius, fixedRadius=np.logspace(-3, 0, 60), r200fac=1, partlim=20, 
	...
	def open_snap(self, snappath, nsnap, filenumber=None):
		if (filenumber is None):# or (self.nfiles==1):
			if os.path.isfile(snappath+"snapshot_%03d.hdf5" %nsnap):
				...
				print("Error: Could not open %ssnapshot_%03d.hdf5" %(snappath, nsnap))
		else:
			if os.path.isfile(snappath+"snapshot_%03d.%i.hdf5" %(nsnap,filenumber+self.nfilestart)):
				...
				print("Error: Could not open %ssnapshot_%03d.%i.hdf5" %(snappath, nsnap, filenumber+self.nfilestart))			
		return dataopen
